...
router.post('/cadastro', (req, res) => {
  try {
    ...
    res.status(201).json({ sucesso: true, token, usuario: { id: result.lastInsertRowid, nome, email, is_admin: 0, role: 'user' } });
  } catch (e) {
...
router.post('/login', (req, res) => {
  try {
    ...
    res.json({ sucesso: true, token, usuario: { id: user.id, nome: user.nome, email: user.email, is_admin: user.is_admin, role: user.is_admin ? 'admin' : 'user' } });
  } catch (e) {
    res.status(500).json({ erro: e.message });
  }
});
