...
body{font-family:Arial;background:#0a0a1a;color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh}
...
if(!s||s.length<6){m.className='msg error';m.textContent='Senha deve ter pelo menos 6 caracteres.';return}
if(s!==c){m.className='msg error';m.textContent='As senhas não conferem.';return}
try{
const r=await fetch('/api/auth/reset',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,s})});
const d=await r.json();
if(r.ok){m.className='msg success';m.textContent='Senha redefinida com sucesso! Volte ao app e faça login.'}
else{m.className='msg error';m.textContent=d.erro||'Erro ao redefinir senha.'}
}catch(e){m.className='msg error';m.textContent='Erro de conexão. Tente novamente.'}
}
</script>
</body>
</html>`);
